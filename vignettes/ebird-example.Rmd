...
head(full_X_env)
```
Let's make sure reticulate works as needed. Note that this will depend on your setup. I'm using reticulate with conda and an environment called `pymc3`, but you will need to change this to your own version.
...
to_add <- paste(colnames(full_X_env)[grep('has_', colnames(full_X_env))], collapse = '+')
